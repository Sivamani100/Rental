...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  // City Configuration & Platform Controls State
  final _minRentController = TextEditingController(text: '2000');
  final _maxRentController = TextEditingController(text: '150000');
  final _supportPhoneController = TextEditingController(text: '+91 94941 23456');
  final _supportWhatsappController = TextEditingController(text: '+91 94941 23456');
  final _supportEmailController = TextEditingController(text: 'support@rentalrajahmundry.in');
  final _newLocalityController = TextEditingController();
  bool _strictVerificationMode = true;
  bool _antiBrokerShield = true;
  bool _directOwnerDirectWhatsapp = true;
  bool _requirePropertyPhotos = true;
  int _maxFreeListingsPerOwner = 3;

  // Banner Studio & Announcements State
  final _bannerTitleController = TextEditingController(text: '🏡 100% Direct Owner Properties');
  final _bannerSubtitleController = TextEditingController(text: 'Save up to ₹25,000 in broker fees. Connect directly with verified homeowners.');
  final _bannerTagController = TextEditingController(text: 'ZERO BROKERAGE');
  final _bannerButtonController = TextEditingController(text: 'Browse Verified Homes');
  int _selectedBannerGradientIndex = 0;
  bool _isEmergencyTickerActive = true;
  final _emergencyTickerTextController = TextEditingController(
    text: '⚡ Anti-Fraud Shield: Never pay advance deposits without inspecting the property in person.',
  );

  List<Map<String, dynamic>> _bannersList = [
    {
      'id': 'b1',
      'title': '🏡 100% Direct Owner Properties',
      'subtitle': 'Save up to ₹25,000 in broker fees. Connect directly with verified homeowners.',
      'tag': 'ZERO BROKERAGE',
      'buttonText': 'Browse 0-Broker Deals',
      'gradient': [AppTheme.primaryYellow, const Color(0xFFD97706)],
      'isActive': true,
      'clicks': 1420,
      'ctr': '18.4%',
    },
    {
      'id': 'b2',
      'title': '🎓 College & Student PG Special',
      'subtitle': 'Top Boys & Girls Hostels near Campus with 3 times homely food & Wi-Fi.',
      'tag': 'CAMPUS HUB',
      'buttonText': 'Explore Student PGs',
    ...
    {
      'id': 'b3',
      'title': '📢 Have a Vacant Flat or PG?',
      'subtitle': 'List in 60 seconds for FREE and connect directly with 5,000+ active tenants.',
      'tag': 'FOR LANDLORDS',
  ...
  // ==========================================
  // TAB 6: HOME BANNERS & ANNOUNCEMENTS STUDIO
  // ==========================================
  Widget _buildBannersTab(bool isDark) {
    ...
    final List<List<Color>> gradientPresets = [
      [AppTheme.primaryYellow, const Color(0xFFD97706)], // Amber Gold
      [const Color(0xFF8B5CF6), const Color(0xFF6D28D9)], // Royal Violet
      [const Color(0xFF10B981), const Color(0xFF047857)], // Emerald Marine
      [const Color(0xFF3B82F6), const Color(0xFF1D4ED8)], // Cyber Ocean
      [const Color(0xFFEC4899), const Color(0xFFBE185D)], // Sunset Crimson
    ];

    final activeBannersCount = _bannersList.where((b) => b['isActive'] == true).length;
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
            ...
            child: Row(
              children: [
                Container(
                  ...
                  alignment: Alignment.center,
                  child: const Icon(Iconsax.gallery_favorite, color: Color(0xFFFFEB3A), size: 24),
                ),
                const SizedBox(width: 14),
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Row(
                        children: [
                          Text(
                            'Home Banners & In-App Announcements',
                            style: GoogleFonts.inter(
                              fontSize: 17,
                              fontWeight: FontWeight.w800,
                              color: primaryTextColor,
                          ...
                          Container(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                const SizedBox(width: 5),
                                Text(
                                  '$activeBannersCount ACTIVE IN APP',
                                  style: GoogleFonts.inter(
                                    fontSize: 10,
                                    fontWeight: FontWeight.w800,
                                    color: const Color(0xFF10B981),
                      ...
                      const SizedBox(height: 4),
                      Text(
                        'Design, customize, and publish high-converting hero banners, student campaigns, and top security tickers in realtime.',
                        style: GoogleFonts.inter(fontSize: 13, color: mutedColor),
                      ),
                    ],
                  ),
                ...
                ElevatedButton.icon(
                  onPressed: () {
                    setState(() {
                      _bannerTitleController.text = '🔥 Weekend Flash Move-In Offer';
                      _bannerSubtitleController.text = 'Zero security deposit on select verified 2BHK and 1BHK listings.';
                      _bannerTagController.text = 'SPECIAL FLASH';
                      _bannerButtonController.text = 'Claim Offer Now';
                      _selectedBannerGradientIndex = 4;
                    });
                    AppSnackbar.success(context, '✨ New template loaded into Studio');
                  },
          ...
          // 2. 4-Core Banner Performance KPI Cards
          LayoutBuilder(
            builder: (context, constraints) {
              final isDesktop = constraints.maxWidth > 900;
              final crossAxisCount = isDesktop ? 4 : (constraints.maxWidth > 550 ? 2 : 1);
              return GridView.count(
                ...
                childAspectRatio: isDesktop ? 2.1 : 2.4,
                children: [
                  // KPI 1: Active Banners
                  _buildExecutiveKpiCard(
                    title: 'Active Carousel Slides',
                    value: '$activeBannersCount Live Slides',
                    subtitle: 'Auto-rotates every 5s in app',
                    badgeLabel: 'Hero Placement',
                    badgeColor: const Color(0xFF10B981),
                  ...
                  // KPI 2: Total Banner Clicks
                  _buildExecutiveKpiCard(
                    title: 'Total In-App Clicks',
                    value: '3,050 Clicks',
                    subtitle: 'Avg 14.8% CTR',
                    badgeLabel: '+22% This Week',
                    badgeColor: const Color(0xFF3B82F6),
                  ...
                  // KPI 3: Security Notice Ticker
                  _buildExecutiveKpiCard(
                    title: 'Top Marquee Ticker',
                    value: _isEmergencyTickerActive ? 'Active & Visible' : 'Paused',
                    subtitle: 'Safety & anti-fraud shield',
                    badgeLabel: _isEmergencyTickerActive ? 'Live Ticker' : 'Offline',
                    badgeColor: _isEmergencyTickerActive ? const Color(0xFF10B981) : const Color(0xFF94A3B8),
                  ...
                  // KPI 4: Top Performing Banner
                  _buildExecutiveKpiCard(
                    title: 'Top Performing Campaign',
                    value: '0 Broker Guarantee',
                    subtitle: '1,420 clicks (18.4% CTR)',
                    badgeLabel: 'Top Converter',
                    badgeColor: const Color(0xFF8B5CF6),
            ...
            builder: (context, constraints) {
              if (constraints.maxWidth > 920) {
                return Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    // Left Column: Active Banners List & Security Ticker Manager
                    Expanded(
                      child: Column(
                        children: [
                          _buildActiveBannersListCard(isDark, primaryTextColor, mutedColor),
                          const SizedBox(height: 16),
                          _buildEmergencyTickerCard(isDark, primaryTextColor, mutedColor),
                        ],
                      ),
                    ),
                    const SizedBox(width: 16),

                    // Right Column: Live Interactive Visual Studio & Form
                    Expanded(
                      child: Column(
                        children: [
                          _buildLiveBannerStudioSimulator(
                            gradientPresets,
                            isDark,
                            primaryTextColor,
                            mutedColor,
  ...
  // ==========================================
  // COMPONENT: ACTIVE BANNERS MATRIX LIST
  // ==========================================
  Widget _buildActiveBannersListCard(bool isDark, Color primaryTextColor, Color mutedColor) {
    return Container(
      padding: const EdgeInsets.all(18),
      decoration: BoxDecoration(
        color: isDark ? const Color(0xFF161922) : Colors.white,
        ...
        children: [
          Row(
            ...
            children: [
              Row(
                children: [
                  const Icon(Iconsax.slider_horizontal, size: 17, color: Color(0xFFFFEB3A)),
                  const SizedBox(width: 8),
                  Text(
                    'Active Home Carousel Slides',
                    style: GoogleFonts.inter(
                      fontSize: 13.5,
                      fontWeight: FontWeight.w800,
                      color: primaryTextColor,
              ...
              Text(
                '${_bannersList.length} Total Registered',
                style: GoogleFonts.inter(fontSize: 11, color: mutedColor),
              ),
            ],
          ),
          ...
          ..._bannersList.asMap().entries.map((entry) {
            final idx = entry.key;
            ...
            return Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 3),
                        Text(
                          '${item['clicks']} clicks  •  ${item['ctr']} CTR',
                          style: GoogleFonts.inter(fontSize: 11, color: mutedColor),
                        ),
                      ],
                    ),
                  ...
                  // Actions: Reuse in studio + Active Switch
                  IconButton(
                    icon: const Icon(Iconsax.edit_2, size: 15),
                    tooltip: 'Load into Studio Editor',
                    color: mutedColor,
                    onPressed: () {
                      setState(() {
                        _bannerTitleController.text = item['title'];
                        _bannerSubtitleController.text = item['subtitle'];
                        _bannerTagController.text = item['tag'];
                        _bannerButtonController.text = item['buttonText'];
                      });
                      AppSnackbar.success(context, '✏️ Loaded "${item['title']}" into Studio');
                    },
                  ),
                  Transform.scale(
                    scale: 0.75,
                    child: Switch(
                      ...
                      onChanged: (val) {
                        ...
                        AppSnackbar.success(
                          context,
                          val ? '🟢 Banner #${idx + 1} activated in Home Carousel' : '⏸️ Banner #${idx + 1} paused',
                        );
                      },
                    ),
                  ),
  ...
  // ==========================================
  // COMPONENT: STICKY EMERGENCY & SECURITY TICKER
  // ==========================================
  Widget _buildEmergencyTickerCard(bool isDark, Color primaryTextColor, Color mutedColor) {
    return Container(
      padding: const EdgeInsets.all(18),
      decoration: BoxDecoration(
        color: isDark ? const Color(0xFF161922) : Colors.white,
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Transform.scale(
                scale: 0.75,
                child: Switch(
                  value: _isEmergencyTickerActive,
                  activeColor: const Color(0xFF10B981),
                  onChanged: (val) {
                    setState(() => _isEmergencyTickerActive = val);
                    AppSnackbar.success(
                      context,
                      val ? '🟢 Top Security Ticker Enabled in App' : '⏸️ Top Ticker Hidden',
                    );
                  },
                ),
              ),
          ...
          const SizedBox(height: 10),
          Text(
            'Displays a persistent security notice strip across all tenant screens.',
            style: GoogleFonts.inter(fontSize: 12, color: mutedColor),
          ),
          const SizedBox(height: 12),

          // Live Ticker Input Field
          TextField(
            controller: _emergencyTickerTextController,
            style: GoogleFonts.inter(fontSize: 12.5, color: primaryTextColor),
            maxLines: 2,
            decoration: InputDecoration(
              filled: true,
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              onPressed: () {
                AppSnackbar.success(context, '⚡ Security ticker broadcasted to all active app sessions!');
              },
  ...
  // ==========================================
  // COMPONENT: LIVE INTERACTIVE BANNER STUDIO SIMULATOR
  // ==========================================
  Widget _buildLiveBannerStudioSimulator(
    List<List<Color>> gradientPresets,
    bool isDark,
    Color primaryTextColor,
    Color mutedColor,
  ) {
    final currentColors = gradientPresets[_selectedBannerGradientIndex];
    final title = _bannerTitleController.text.isEmpty ? '🏡 100% Direct Owner Properties' : _bannerTitleController.text;
    final subtitle = _bannerSubtitleController.text.isEmpty
        ? 'Save up to ₹25,000 in broker fees. Connect directly with verified homeowners.'
        : _bannerSubtitleController.text;
    final tag = _bannerTagController.text.isEmpty ? 'ZERO BROKERAGE' : _bannerTagController.text;
    final btnText = _bannerButtonController.text.isEmpty ? 'Browse Verified Homes' : _bannerButtonController.text;
        ...
        children: [
          Row(
            ...
            children: [
              Row(
                children: [
                  const Icon(Iconsax.magicpen, size: 17, color: Color(0xFF8B5CF6)),
                  const SizedBox(width: 8),
                  Text(
                    'Live Banner Studio & Simulator',
                    style: GoogleFonts.inter(
                      fontSize: 13.5,
                      fontWeight: FontWeight.w800,
                      color: primaryTextColor,
              ...
              Container(
                ...
                child: Text(
                  'REALTIME PREVIEW',
                  style: GoogleFonts.inter(fontSize: 9.5, fontWeight: FontWeight.w800, color: const Color(0xFF8B5CF6)),
                ),
              ),
            ],
          ),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [
                BoxShadow(
                  color: currentColors.first.withValues(alpha: 0.35),
                  blurRadius: 18,
          ...
          // 2. Gradient Color Theme Palette Picker
          Text(
            'Select Gradient Color Palette:',
            style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w700, color: primaryTextColor),
          ),
          const SizedBox(height: 8),
          Row(
            children: gradientPresets.asMap().entries.map((entry) {
              final idx = entry.key;
              final colors = entry.value;
              final isSelected = _selectedBannerGradientIndex == idx;

              return Padding(
                padding: const EdgeInsets.only(right: 10),
                child: InkWell(
                  borderRadius: BorderRadius.circular(8),
                  onTap: () => setState(() => _selectedBannerGradientIndex = idx),
                  child: Container(
                    width: 38,
                    height: 38,
                    decoration: BoxDecoration(
          ...
          // 3. Form Inputs
          Text('Tag / Pill Label:', style: GoogleFonts.inter(fontSize: 11.5, color: mutedColor)),
          const SizedBox(height: 4),
          TextField(
            controller: _bannerTagController,
            onChanged: (_) => setState(() {}),
            style: GoogleFonts.inter(fontSize: 12.5, color: primaryTextColor),
            decoration: _buildInputDecoration(isDark, 'e.g. ZERO BROKERAGE'),
          ),
          const SizedBox(height: 10),

          Text('Banner Headline:', style: GoogleFonts.inter(fontSize: 11.5, color: mutedColor)),
          const SizedBox(height: 4),
          TextField(
            controller: _bannerTitleController,
            onChanged: (_) => setState(() {}),
            style: GoogleFonts.inter(fontSize: 12.5, color: primaryTextColor),
            decoration: _buildInputDecoration(isDark, 'e.g. 🏡 100% Direct Owner Properties'),
          ),
          const SizedBox(height: 10),

          Text('Subtitle / Offer Details:', style: GoogleFonts.inter(fontSize: 11.5, color: mutedColor)),
          const SizedBox(height: 4),
          TextField(
            controller: _bannerSubtitleController,
            onChanged: (_) => setState(() {}),
            style: GoogleFonts.inter(fontSize: 12.5, color: primaryTextColor),
            decoration: _buildInputDecoration(isDark, 'e.g. Save up to ₹25,000 in broker fees.'),
          ),
          const SizedBox(height: 10),

          Text('Button CTA Text:', style: GoogleFonts.inter(fontSize: 11.5, color: mutedColor)),
          const SizedBox(height: 4),
          TextField(
            controller: _bannerButtonController,
            onChanged: (_) => setState(() {}),
            style: GoogleFonts.inter(fontSize: 12.5, color: primaryTextColor),
            decoration: _buildInputDecoration(isDark, 'e.g. Browse Verified Homes'),
          ),
          const SizedBox(height: 16),
            ...
            child: ElevatedButton.icon(
              onPressed: () {
                final newBanner = {
                  ...
                  'tag': _bannerTagController.text.trim().toUpperCase(),
                  'buttonText': _bannerButtonController.text.trim(),
                ...
                setState(() {
                  _bannersList.insert(0, newBanner);
                });
                AppSnackbar.success(context, '🚀 Successfully published banner to Home Carousel!');
              },
              icon: const Icon(Iconsax.send_1, size: 16),
              label: Text(
                'Publish Banner to App Carousel',
  ...
  Widget _buildSettingsTab(bool isDark) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // 1. City Master Operational Status Banner
          Container(
            padding: const EdgeInsets.all(18),
            decoration: BoxDecoration(
              color: isDark ? const Color(0xFF161922) : Colors.white,
            ...
            child: Row(
              children: [
                Container(
                  ...
                  alignment: Alignment.center,
                  child: const Icon(Iconsax.global, color: Color(0xFFFFEB3A), size: 22),
                ),
                const SizedBox(width: 14),
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Row(
                        children: [
                          Text(
                            'City Configurations & Platform Settings',
                            style: GoogleFonts.inter(
                              fontSize: 16,
                              fontWeight: FontWeight.w800,
                              color: primaryTextColor,
                      ...
                      const SizedBox(height: 3),
                      Text(
                        'Manage operational coverage zones, pricing limits, verification policies, and anti-broker guardrails.',
                        style: GoogleFonts.inter(fontSize: 12, color: mutedColor),
                      ),
                    ],
                  ),
                ...
                ElevatedButton.icon(
                  onPressed: () {
                    AppSnackbar.success(context, '✅ Platform configurations saved successfully!');
                  },
                  icon: const Icon(Iconsax.tick_circle, size: 16),
                  label: Text('Save All Changes', style: GoogleFonts.inter(fontSize: 12.5, fontWeight: FontWeight.w700)),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: AppTheme.primaryYellow,
                    foregroundColor: Colors.black,
                    elevation: 0,
            ...
            builder: (context, constraints) {
              if (constraints.maxWidth > 980) {
                return Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Expanded(flex: 3, child: _buildLeftSettingsColumn(isDark, primaryTextColor, mutedColor)),
                    const SizedBox(width: 16),
                    Expanded(flex: 2, child: _buildRightSettingsColumn(isDark, primaryTextColor, mutedColor)),
                  ],
                );
              } else {
                return Column(
                  children: [
                    _buildLeftSettingsColumn(isDark, primaryTextColor, mutedColor),
                    const SizedBox(height: 16),
                    _buildRightSettingsColumn(isDark, primaryTextColor, mutedColor),
                  ],
                );
              }
            },
  ...
  Widget _buildLeftSettingsColumn(bool isDark, Color primaryTextColor, Color mutedColor) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        // 2. Interactive Locality Management Matrix
        Container(
          padding: const EdgeInsets.all(18),
          decoration: BoxDecoration(
            color: isDark ? const Color(0xFF161922) : Colors.white,
            ...
            children: [
              Row(
                ...
                children: [
                  Row(
                    children: [
                      const Icon(Iconsax.location, size: 17, color: Color(0xFFFFEB3A)),
                      const SizedBox(width: 8),
                      Text(
                        'Operational Localities (${_rajahmundryLocalities.length})',
                        style: GoogleFonts.inter(
                          fontSize: 14,
                          fontWeight: FontWeight.w800,
                          color: primaryTextColor,
                ...
                children: [
                  Expanded(
                    child: Container(
                      ...
                      child: TextField(
                        controller: _newLocalityController,
                        style: GoogleFonts.inter(fontSize: 12.5, color: primaryTextColor),
                        decoration: InputDecoration(
                          hintText: 'Add new locality (e.g. Sambamurthy Nagar)...',
                          hintStyle: GoogleFonts.inter(fontSize: 12, color: mutedColor.withValues(alpha: 0.7)),
                  ...
                  InkWell(
                    borderRadius: BorderRadius.circular(8),
                    onTap: () {
                      final newLoc = _newLocalityController.text.trim();
                      if (newLoc.isNotEmpty) {
                        if (!_rajahmundryLocalities.contains(newLoc)) {
                          setState(() {
                            _rajahmundryLocalities.add(newLoc);
                            _newLocalityController.clear();
                          });
                          AppSnackbar.success(context, '📍 Added "$newLoc" to Rajahmundry localities!');
                        } else {
                          AppSnackbar.error(context, 'Locality "$newLoc" already exists.');
                        }
                      }
                    },
                    child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        InkWell(
                          borderRadius: BorderRadius.circular(4),
                          onTap: () {
                            if (_rajahmundryLocalities.length > 1) {
                              setState(() => _rajahmundryLocalities.remove(loc));
                              AppSnackbar.success(context, 'Removed "$loc"');
                            }
                          },
                          child: Padding(
                            padding: const EdgeInsets.all(2.0),
                            child: Icon(Icons.close, size: 13, color: mutedColor),
                          ),
                        ),
                      ],
                    ),
        ...
        // 3. Rental Pricing & Quality Guardrails
        Container(
          padding: const EdgeInsets.all(18),
          decoration: BoxDecoration(
            color: isDark ? const Color(0xFF161922) : Colors.white,
            ...
            children: [
              Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Text(
                    'Listing Verification & Pricing Guardrails',
                    style: GoogleFonts.inter(
                      fontSize: 14,
                      fontWeight: FontWeight.w800,
                      color: primaryTextColor,
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          'Min Rent Limit (₹/mo)',
                          style: GoogleFonts.inter(fontSize: 11.5, fontWeight: FontWeight.w600, color: mutedColor),
                        ),
                        const SizedBox(height: 4),
                        Container(
                          height: 38,
              ...
              // Switch 1: Strict Verification Mode
              SwitchListTile.adaptive(
                contentPadding: EdgeInsets.zero,
                value: _strictVerificationMode,
                activeColor: AppTheme.primaryYellow,
                title: Text(
                  'Strict Admin Verification Gate',
                  style: GoogleFonts.inter(fontSize: 12.5, fontWeight: FontWeight.w700, color: primaryTextColor),
                ),
                subtitle: Text(
                  'Hold new owner submissions in "Pending" queue until reviewed by Rajahmundry admin.',
                  style: GoogleFonts.inter(fontSize: 11, color: mutedColor),
                ),
                onChanged: (val) => setState(() => _strictVerificationMode = val),
              ),
              const Divider(height: 14),

              // Switch 2: Require Real Room Photos
              SwitchListTile.adaptive(
                contentPadding: EdgeInsets.zero,
                value: _requirePropertyPhotos,
                activeColor: AppTheme.primaryYellow,
                title: Text(
                  'Require Room Photos for Approval',
                  style: GoogleFonts.inter(fontSize: 12.5, fontWeight: FontWeight.w700, color: primaryTextColor),
                ),
                subtitle: Text(
                  'Prevent zero-photo property submissions from appearing on tenant feeds.',
                  style: GoogleFonts.inter(fontSize: 11, color: mutedColor),
                ),
                onChanged: (val) => setState(() => _requirePropertyPhotos = val),
              ),
            ],
          ),
        ),
  ...
  Widget _buildRightSettingsColumn(bool isDark, Color primaryTextColor, Color mutedColor) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        // 4. Broker Shield & Lead Routing
        Container(
          padding: const EdgeInsets.all(18),
          decoration: BoxDecoration(
            color: isDark ? const Color(0xFF161922) : Colors.white,
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  const Icon(Iconsax.lock, size: 17, color: Color(0xFF8B5CF6)),
                  const SizedBox(width: 8),
                  Text(
                    'Zero-Brokerage Protection & Security',
                    style: GoogleFonts.inter(
                      fontSize: 14,
                      fontWeight: FontWeight.w800,
                      color: primaryTextColor,
              ...
              SwitchListTile.adaptive(
                contentPadding: EdgeInsets.zero,
                value: _antiBrokerShield,
                activeColor: AppTheme.primaryYellow,
                title: Text(
                  'Anti-Broker Keyword Shield',
                  style: GoogleFonts.inter(fontSize: 12.5, fontWeight: FontWeight.w700, color: primaryTextColor),
                ),
                subtitle: Text(
                  'Automatically flag descriptions containing "brokerage", "commission", or "agent charge".',
                  style: GoogleFonts.inter(fontSize: 11, color: mutedColor),
                ),
                onChanged: (val) => setState(() => _antiBrokerShield = val),
              ),
              const Divider(height: 14),

              SwitchListTile.adaptive(
                contentPadding: EdgeInsets.zero,
                value: _directOwnerDirectWhatsapp,
                activeColor: AppTheme.primaryYellow,
                title: Text(
                ...
                subtitle: Text(
                  'Let verified tenants message landlords directly with zero platform intermediary delays.',
                  style: GoogleFonts.inter(fontSize: 11, color: mutedColor),
                ),
                onChanged: (val) => setState(() => _directOwnerDirectWhatsapp = val),
              ),
            ],
          ),
        ),
        ...
        // 5. Rajahmundry Helpline & Support Contacts
        Container(
          padding: const EdgeInsets.all(18),
          decoration: BoxDecoration(
            color: isDark ? const Color(0xFF161922) : Colors.white,
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  const Icon(Iconsax.call, size: 17, color: Color(0xFF3B82F6)),
                  const SizedBox(width: 8),
                  Text(
                    'Rajahmundry Admin & Helpdesk Contacts',
                    style: GoogleFonts.inter(
                      fontSize: 14,
                      fontWeight: FontWeight.w800,
                      color: primaryTextColor,
              ...
              Text('Support Phone Number', style: GoogleFonts.inter(fontSize: 11.5, fontWeight: FontWeight.w600, color: mutedColor)),
              const SizedBox(height: 4),
              Container(
                height: 38,
                decoration: BoxDecoration(
                ...
                child: TextField(
                  ...
                  style: GoogleFonts.inter(fontSize: 12.5, color: primaryTextColor),
                  decoration: const InputDecoration(
                    prefixIcon: Icon(Icons.phone_rounded, size: 15, color: Color(0xFF3B82F6)),
                    contentPadding: EdgeInsets.symmetric(horizontal: 10, vertical: 10),
                    border: InputBorder.none,
                  ),
                ),
              ...
              Text('Support Email', style: GoogleFonts.inter(fontSize: 11.5, fontWeight: FontWeight.w600, color: mutedColor)),
              const SizedBox(height: 4),
              Container(
                height: 38,
                decoration: BoxDecoration(
                ...
                child: TextField(
                  ...
                  style: GoogleFonts.inter(fontSize: 12.5, color: primaryTextColor),
                  decoration: const InputDecoration(
                    prefixIcon: Icon(Icons.email_outlined, size: 15, color: Color(0xFF3B82F6)),
                    contentPadding: EdgeInsets.symmetric(horizontal: 10, vertical: 10),
                    border: InputBorder.none,
                  ),
                ),
        ...
        // 6. Upcoming Expansion Cities (Multi-City Pipeline)
        Container(
          padding: const EdgeInsets.all(18),
          decoration: BoxDecoration(
            color: isDark ? const Color(0xFF161922) : Colors.white,
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  Text('Andhra Pradesh', style: GoogleFonts.inter(fontSize: 11, color: mutedColor)),
                ],
              ),
              const SizedBox(height: 10),
              _buildCityExpansionTile('Kakinada', 'Smart City & Port Hub', 'Launch Q3 2026', isDark, primaryTextColor, mutedColor),
              const SizedBox(height: 6),
              _buildCityExpansionTile('Vijayawada', 'Commercial & IT Capital', 'Launch Q4 2026', isDark, primaryTextColor, mutedColor),
              const SizedBox(height: 6),
              _buildCityExpansionTile('Visakhapatnam', 'Beach City & Tech Corridor', 'Launch Q1 2027', isDark, primaryTextColor, mutedColor),
            ],
          ),
        ),
      ],
  ...
  Widget _buildCityExpansionTile(
    ...
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.location_city_rounded, size: 16, color: Color(0xFFFFEB3A)),
          const SizedBox(width: 8),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(cityName, style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w700, color: primaryTextColor)),
                Text(desc, style: GoogleFonts.inter(fontSize: 10.5, color: mutedColor)),
              ],
            ),
          ),
          Container(
            ...
            child: Text(
              launchTimeline,
              style: GoogleFonts.inter(fontSize: 9.5, fontWeight: FontWeight.w700, color: const Color(0xFF3B82F6)),
            ),
          ),
        ],
      ),
